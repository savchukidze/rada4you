...
## Огляд 
Це бібліотека для роботи з API порталу "Вони голосують для тебе". Вона має 10 базових функцій. Що вони робить?

* **`mps("key")`** - видає датафрейм із базовою інформацією про кожного народного депутата.
* **`mps_info(id, "key")`** – видає датафрейм із деталізованою інформацією про кожного народного депутата.
* **`policies("key")`** – видає датафрейм із базовою інформацію про усі політики.
* **`policy_details(policy_id, "key")`** – видає датафрейм із голосуваннями, що включені до певної політики.
* **`mps_policies(id, "key")`** – видає датафрейм із рівнем підтримки певного депутата кожної з політик.
...
* **`voting_mps(id, "key")`** – видає датафрейм із голосуванням кожного з депутатів щодо певного голосування.
* **`voting_policies(id, "key")`** – видає датафрейм із сукупністю політик, до яких підв'язане певне голосування.
...
``` r
mps_policies(id, "key") – видає датафрейм із рівнем певного депутата кожної політик.
```
|Параметр|Опис|
| --- | --- |
| `mp_id` | Унікальний визначник для кожного депутата |
...
``` r
voting_info(id, "key") – видає датафрейм із деталізованою інформацією про кожне голосування.
```
|Параметр|Опис|
| --- | --- |
| `vote_id` | Унікальний визначник для голосування |
